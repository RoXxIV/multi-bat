...
int modbus_read_registers(uint8_t slave_id, uint16_t start_addr, uint16_t reg_count, uint8_t *dest_buffer)
{
    ...
    // 1. Construire la trame de commande
    ...
    unsigned long timeout = millis() + 400; // Timeout augmenté à 750ms -vvv
    ...
    // La taille attendue est 5 + (nombre de registres * 2)
    int expected_len = 5 + reg_count * 2;
    if (responseLength != expected_len)
        return -1; // Taille incorrecte
    ...
    if (receivedCrc != calculatedCrc)
        return -1; // CRC incorrect

    // Si tout est OK, on copie les données utiles (la "payload") dans le buffer de destination
...
bool modbus_write_register(uint8_t slave_id, uint16_t reg_addr, uint16_t value)
{
    ...
    uint8_t response[32];
    unsigned long timeout = millis() + 400; // Timeout augmenté -vvv
    int n = 0;
    ...
    return false; // Timeout ou réponse incorrecte
}
...
bool modbus_write_multiple_registers(uint8_t slave_id, uint16_t start_addr, uint16_t reg_count, uint8_t *data)
{
    ...
    // 2. Envoyer la commande (inchangé)
    while (_modbusSerial->available())
        _modbusSerial->read();
    modbus_enable_transmit();
    ...
    if (n == 8)
    {
        if (response[0] == (0x50 + slave_id) && response[1] == CMD_WRITE_MULTIPLE &&
    ...
    // On garde notre espion en cas de problème futur
    modbus_print_buffer("DEBUG: ACK Reçu (échec validation)", response, n);
    return false;
}